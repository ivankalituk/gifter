...
export async function getAllGiftsByTags(tags: string[], sort: string, byName: string, user_id: number):Promise<Gift[] | undefined> {
    try{
        return await axios.post<Gift[] | undefined>(serverUrl + '/gift/tags', {tags: tags, sort: sort, byName: byName, user_id: user_id}).then(({data}) => data);
    } catch (error){
        if (axios.isAxiosError(error)){
