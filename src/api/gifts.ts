...
export async function getAllGifts(): Promise<Gift[]> {
    try {
        const response = await axios.get<{ data: Gift[] }>('http://192.168.0.105:1000/gift').then(({data}) => data);
...
export async function getGiftNameByName(name: string): Promise<giftName[]> {
    try {
        const response = await axios.post<giftName[]>('http://192.168.0.105:1000/gift/name', {name: name});
...
export async function getAllGiftsByTags(tags: string[], sort: string, byName: string):Promise<Gift[] | undefined> {
    try{
        console.log(byName)
        return await axios.post<Gift[] | undefined>('http://192.168.0.105:1000/gift/tags', {tags: tags, sort: sort, byName: byName}).then(({data}) => data);
    } catch (error){
        if (axios.isAxiosError(error)){
