...
export async function putUserNickname(data: any) {
    try{
        return await axios.put('http://192.168.0.105:1000/user/nickname', {nickname: data.nickname, user_id: data.user_id})
    } catch (error){
        console.log("ERROR WHITE GETTING DATA")
    }
}
...
export async function putUserBio(data: any) {
    try{
        return await axios.put('http://192.168.0.105:1000/user/bio', {bio: data.bio, user_id: data.user_id})
    } catch (error){
        console.log("ERROR WHITE GETTING DATA")
    }
}
...
export async function putUserTags(data: any) {
    try{
        return await axios.put('http://192.168.0.105:1000/user/tags', {tags: data.tags, user_id: data.user_id})
    } catch (error){
        console.log("ERROR WHITE GETTING DATA")
    }
}
...
export async function getUserTags(data:any) {
    try{
        const response = await axios.get('http://192.168.0.105:1000/user/tags/' + data.user_id);
...
export async function putUserPhoto(data: any) {
    try{
        return await axios.put('http://192.168.0.105:1000/user/photo', data, {
